...
public class LongJump extends Module {
    ...
    public void onPreUpdate(PreUpdateEvent e) {
        ...
        if (stopTime == -1 && ++boostTicks > (!temporaryFlightKey() ? 33 : (!notMoving ? 32 : 33))) {
            disabled();
            return;
        }
    ...
    private int setupFireballSlot(boolean pre) {
        ...
        if (fireballSlot == -1) {
            ...
            disabled();
        } else if (pre && Utils.distanceToGround(mc.thePlayer) > 3) {
    ...
    private void modifyVertical() {
        if (verticalMotion.getInput() != 0) {
            double ver = ((!notMoving ? verticalMotion.getInput() : 1.16 ) * (1.0 / (1.0 + (0.05 * getSpeedLevel())))) + Utils.randomizeDouble(0.0001, 0.1);
            double decay = motionDecay.getInput() / 1000;
            if (boostTicks > 1 && !temporaryFlightKey()) {
                if (boostTicks > 1 || boostTicks <= (!notMoving ? 32 : 33)) {
                    mc.thePlayer.motionY = Utils.randomizeDouble(0.0101, 0.01);
                }
            } else {
                if (boostTicks >= 1 && boostTicks <= (!notMoving ? 32 : 33)) {
                    mc.thePlayer.motionY = ver - boostTicks * decay;
                } else if (boostTicks >= (!notMoving ? 32 : 33) + 3) {
