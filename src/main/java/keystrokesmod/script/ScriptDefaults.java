...
public class ScriptDefaults {
    ...
    public static class modules {
        ...
        public String getSliderString(String moduleName, String name) {
            SliderSetting setting = ((SliderSetting) getSetting(getModule(moduleName), name));
            if (setting == null || setting.getOptions() == null) {
                return "";
            }
            int index = (int) Math.round(setting.getInput());
            if (index < 0 || index >= setting.getOptions().length) {
                return "";
            }
            return setting.getOptions()[index];
        }

        public Vec3 getWhitelistedBedPos() {
            if (ModuleManager.bedAura != null && ModuleManager.bedAura.getWhitelistedBedPos() != null) {
                BlockPos pos = ModuleManager.bedAura.getWhitelistedBedPos();
