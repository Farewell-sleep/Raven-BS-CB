...
public class Script {
    ...
    public boolean run() {
        try {
            if (this.scriptName == null || this.codeStr == null) {
                Utils.sendDebugMessage("§cScript load failed §7(" + Utils.extractFileName(this.name) + ") §8- missing script source");
                return false;
            }
            ...
            if (Raven.scriptManager.compiler == null) {
                Utils.sendDebugMessage("§cScript compiler unavailable §7(" + Utils.extractFileName(this.name) + ")");
                Utils.sendDebugMessage(" §7reason: §cJDK tools not found (ToolProvider.getSystemJavaCompiler returned null)");
                return false;
            }
            ...
            if (!(boolean) Launch.blackboard.get("fml.deobfuscatedEnvironment")) {
                ...
                compilationOptions.add(s + File.pathSeparator + System.getProperty("java.class.path"));
            }
            else {
                compilationOptions.add("-classpath");
                compilationOptions.add(System.getProperty("java.class.path"));
            }
            ...
            if (!success) {
                this.error = true;
                Utils.sendDebugMessage("§cScript compile failed §7(" + Utils.extractFileName(this.name) + ")");
                return false;
            }
            try {
            ...
            this.error = true;
            Utils.sendDebugMessage("§cUnexpected script load error §7(" + Utils.extractFileName(this.name) + ") §8- " + ex.getClass().getSimpleName());
            return false;
        }
    }

    public int getBoolean(final String s, final Object... array) {
        Method method = findMethod(s, array.length, null);
        if (method != null) {
            try {
                method.setAccessible(true);
                final Object result = method.invoke(this.instance, prepareArguments(method, array));
    ...
    public Float[] getFloatArray(String methodName, Object... args) {
        Method method = findMethod(methodName, args.length, null);
        if (method != null) {
            try {
                method.setAccessible(true);
                Object result = method.invoke(this.instance, prepareArguments(method, args));
                if (result instanceof Float[]) {
                    return (Float[]) result;
                } else if (result instanceof float[]) {
                    float[] prim = (float[]) result;
                    Float[] boxed = new Float[prim.length];
    ...
    public boolean invoke(final String s, final Object... array) {
        ...
        if (method != null) {
            try {
                method.setAccessible(true);
                method.invoke(this.instance, prepareArguments(method, array));
                return true;
            }
            catch (IllegalAccessException | InvocationTargetException ex) {
                ReflectiveOperationException er = ex;
    ...
    private Method findMethod(String name, int argCount, Class<?> returnType) {
        if (this.clazz == null) {
            return null;
        }
        ...
        for (Method method : this.clazz.getDeclaredMethods()) {
            if (method.getName().equalsIgnoreCase(name)) {
                ...
                if (bestMethod == null || method.getParameterCount() < argCount) {
                    bestMethod = method;
                }
            }
        }
    ...
    private Object[] prepareArguments(Method method, Object... array) {
        int paramCount = method.getParameterCount();
        if (paramCount == array.length) {
            return array;
        } else if (paramCount < array.length) {
            return Arrays.copyOf(array, paramCount);
        } else {
            ...
            for (int i = 0; i < paramCount; i++) {
                if (i < array.length) {
                    ...
                    if (type == boolean.class) {
                        padded[i] = false;
                    } else if (type == int.class || type == short.class || type == byte.class) {
                        padded[i] = 0;
