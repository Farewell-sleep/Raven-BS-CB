...
public class KillAura extends Module {
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) OpenMyau.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    ...
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (OpenMyau.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            OpenMyau.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                    ...
                    switch (this.autoBlock.getValue()) {
                        case 0: // NONE
                            if (PlayerUtil.isUsingItem()) {
                                ...
                                if (!this.isPlayerBlocking() && !OpenMyau.playerStateManager.digging && !OpenMyau.playerStateManager.placing) {
                                    swap = true;
                                }
                            } else {
                                this.isBlocking = false;
                        ...
                        case 2: // SPOOF
                            if (this.hasValidTarget()) {
                                ...
                                if (OpenMyau.playerStateManager.digging
                            ...
                            if (this.hasValidTarget()) {
                                if (!OpenMyau.playerStateManager.digging && !OpenMyau.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        ...
                                        case 1:
                                            if (this.isPlayerBlocking()) {
                                                if(OpenMyau.moduleManager.modules.get(NoSlow.class).isEnabled()){
                        ...
                        case 5: // INTERACT
                            if (this.hasValidTarget()) {
                                int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                if (mc.thePlayer.inventory.currentItem == item && !OpenMyau.playerStateManager.digging && !OpenMyau.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        case 0:
                                            if (!this.isPlayerBlocking()) {
                                                swap = true;
                        ...
                        case 8: // FAKE
                            ...
                            if (PlayerUtil.isUsingItem()
                                    ...
                                    && !OpenMyau.playerStateManager.digging
                                    && !OpenMyau.playerStateManager.placing) {
                                swap = true;
                            }
                    }
            ...
            if (this.showTarget.getValue() != 0
                ...
                switch (this.showTarget.getValue()) {
                    case 1:
                        ...
                        break;
                    case 2:
                        color = ((HUD) OpenMyau.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
                }
